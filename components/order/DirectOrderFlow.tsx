...
export function DirectOrderFlow() {
  ...
  const [selectedStyleId, setSelectedStyleId] = useState<string>(DEFAULT_STYLES[0]?.id || '');
  const [styleError, setStyleError] = useState<string | undefined>();

  const [selectedPackageId, setSelectedPackageId] = useState<string>(
    DEFAULT_PACKAGES.find((p) => p.is_popular)?.id || DEFAULT_PACKAGES[0]?.id || ''
  );
  ...
  // Carregar catálogo do banco / API
  useEffect(() => {
    fetch('/api/catalog')
      .then((res) => res.json())
      .then((data) => {
        ...
        if (data.styles && data.styles.length > 0) setStyles(data.styles);
      })
      .catch(() => {});
  }, []);

  // Pré-seleção inteligente via query params
  useEffect(() => {
  ...
  const availableStyles = styles.filter(
    (s) => (s.category_id === selectedCategoryId || s.category_id.includes(selectedCategoryId)) && s.is_active
  );

  // Ao selecionar uma categoria, auto-seleciona o primeiro estilo correspondente
  const handleCategorySelect = (categoryId: string) => {
    setSelectedCategoryId(categoryId);
    setCategoryError(undefined);

    const matchingStyles = styles.filter(
      (s) => (s.category_id === categoryId || s.category_id.includes(categoryId)) && s.is_active
    );
  ...
  const handleSubmitOrder = async () => {
    ...
    const matchingStyles = styles.filter((s) => s.category_id === catId);
    const styId = selectedStyleId || matchingStyles[0]?.id || styles[0]?.id;
    const pkgId = selectedPackageId || packages.find((p) => p.is_popular)?.id || packages[0]?.id;
  ...
  const currentStyle = styles.find((s) => s.id === selectedStyleId) || availableStyles[0] || styles[0];
      ...
      <main className="flex-1 py-6 sm:py-10">
        <div className="max-w-3xl mx-auto px-4 sm:px-6">
          ...
          <div className="bg-[#FFFDF9] border border-[#E6E1D8] rounded-3xl p-6 sm:p-8 shadow-sm">
            ...
            {currentStep === 4 && (
              <StyleStep
                styles={availableStyles.length > 0 ? availableStyles : styles}
